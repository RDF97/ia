...
export default function Luz() {
  ...
  if (!data) {
    return (
      <SafeAreaView className="flex-1 bg-bg items-center justify-center" edges={["top"]}>
        ...
        <Text className="text-secondary mt-3">Cargando precios…</Text>
      </SafeAreaView>
    );
  }
  ...
  const nowColor = nowTier === "ok" ? "#34C759" : nowTier === "mid" ? "#FF9500" : "#FF3B30";
    ...
    <SafeAreaView className="flex-1 bg-bg" edges={["top"]}>
      ...
      >
        ...
        <View className="bg-card rounded-lg2 mx-4 mb-3 overflow-hidden" style={cardShadow(t.dark)}>
          {stretches.map((r, i) => (
            <View key={i} className="flex-row items-center px-4 py-3" style={{ borderTopWidth: i ? 0.5 : 0, borderTopColor: t.separator }}>
              <View className="rounded-lg items-center justify-center mr-3" style={{ width: 32, height: 32, backgroundColor: t.green }}>
...
function AlertRow({
  ...
  const t = useTheme();
  return (
    <View className="flex-row items-center px-3.5 py-2.5" style={{ gap: 12, borderTopWidth: first ? 0 : 0.5, borderTopColor: t.separator }}>
